...
import type { QuizFixtureId } from "../../src/game/quiz/fixtures";
import { E2E_BASE_URL, getE2eArtifactDir } from "./e2e.config";

export class QuizFixturePage {
    ...
    async gotoFixture(
        ...
        const url = new URL("/dev/quiz", E2E_BASE_URL);
        ...
        await this.page.waitForSelector('[data-testid="quiz-room"]');
    ...
    async sentMessages() {
        return this.page.evaluate(() => window.__QUIZ_FIXTURE__?.sentMessages ?? []);
    }

    async hostActions() {
        return this.page.evaluate(() => window.__QUIZ_FIXTURE__?.hostActions ?? []);
    }
    ...
    async takeScreenshot(name: string, testId?: string) {
        const mode = process.env.E2E_UPDATE_SCREENSHOTS === "1"
            ? "baseline"
            : "current";
        const outputDir = path.join(getE2eArtifactDir("quiz"), mode);
    ...
    private async waitForWindowState() {
        ...
        while (Date.now() < timeoutAt) {
            const hasState = await this.page.evaluate(
                () => Boolean(window.__QUIZ_FIXTURE__),
            );
            if (hasState) return;

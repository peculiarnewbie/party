...
import type { BlackjackFixtureId } from "../../src/game/blackjack/fixtures";
import { E2E_BASE_URL, getE2eArtifactDir } from "./e2e.config";

export class BlackjackFixturePage {
    ...
    async gotoFixture(
        ...
        const url = new URL("/dev/blackjack", E2E_BASE_URL);
    ...
    async sentMessages() {
        return this.page.evaluate(
            () => window.__BLACKJACK_FIXTURE__?.sentMessages ?? [],
        );
    }

    async hostActions() {
        return this.page.evaluate(
            () => window.__BLACKJACK_FIXTURE__?.hostActions ?? [],
        );
    }
    ...
    async takeScreenshot(name: string, testId?: string) {
        const mode =
            process.env.E2E_UPDATE_SCREENSHOTS === "1"
                ? "baseline"
                : "current";
        const outputDir = path.join(getE2eArtifactDir("blackjack"), mode);
    ...
    private async waitForWindowState() {
        ...
        while (Date.now() < timeoutAt) {
            const hasState = await this.page.evaluate(() =>
                Boolean(window.__BLACKJACK_FIXTURE__),
            );
            if (hasState) return;
